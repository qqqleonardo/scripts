...
// 定义常量
const BASE_URL = 'https://dkmall.51pt.top/app/ys';
const APP_NAME = '杜康会员俱乐部小程序';
...
async function main() {
    const token = $.getdata("DKHY");
    // 签到
    console.log("\n开始签到");
    console.log("——————");
    const body = {};
    try {
        const sign = await commonPost('/signIn/save', token, body);
        if (sign.code === 200) {
            console.log("签到成功");
            notice += `用户签到成功\n`;
        } else {
        ...
        console.log(`签到请求出错: ${error.message}`);
        notice += `签到请求出错: ${error.message}\n`;
    }
    ...
    console.log("\n——————");
    try {
        const info = await commonGet("/mine/getMemberInfo", token);
        console.log(`当前拥有积分: ${info.data.member.score}\n`);
        notice += `用户当前拥有积分: ${info.data.member.score}\n`;
    } catch (error) {
        console.log(`获取信息请求出错: ${error.message}`);
        notice += `获取信息请求出错: ${error.message}\n`;
    }
...
async function sendMsg(message) {
    if ($.isNode()) {
        let notify;
        try {
            notify = await import('./sendNotify.mjs').then(module => module.default);
        } catch (e) {
            notify = await import('../sendNotify.mjs').then(module => module.default);
        }
